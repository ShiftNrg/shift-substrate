...
useDocker() {
    ...
    docker run parity/subkey:2.0.0-rc6 generate -n "shift" --scheme Sr25519

    printf "Generating Granpa Keys...\n"
    read -p 'Paste Mnemoic: ' mnemonic
    docker run parity/subkey:2.0.0-rc6 inspect-key -n "shift" --scheme Ed25519 "$mnemonic"
...
useSubkey() {
    ...
    printf "Generating Aura Keys\n"
    subkey generate -n "shift" --scheme Sr25519

    printf "Generating Granpa Keys...\n"
    read -p 'Paste Mnemoic: ' mnemonic
    subkey inspect-key -n "shift" --scheme Ed25519 "$mnemonic"
}
